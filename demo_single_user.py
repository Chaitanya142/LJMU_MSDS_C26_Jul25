...
───────────────
  ...
  STAGE 3  Risk scoring & labelling  (sign-corrected PCA PC1 → bell-curve bin)
...
def stage3_risk_label(processed_df: pd.DataFrame, customer_id: str) -> tuple[str, float]:
    ...
    _section("PCA PC1 Risk Score computation")
    print(f"    Formula  : sign_factor × pca.transform(MinMaxScaled(X))[:, 0]")
...
def stage5_fl_model(processed_df: pd.DataFrame, customer_id: str, risk_label: str) -> str:
    ...
    print(f"    Privacy   : Differentially-private FL  (ε=0.4862, δ=1e-5)  [Excellent ε<1]")
...
def stage6_recommendations(risk_label: str, mf_df: pd.DataFrame,
    ...
    _section("Equity–Debt glide path (3-year SEBI-aligned allocation)")
    glide = [(3, 50, 50)]
        ...
        for bkt in port.get("brackets", []):
            ...
            if funds_df is not None and not (hasattr(funds_df, 'empty') and funds_df.empty):
                ...
                for i, f in enumerate(rows, 1):
                    ...
                    seg    = f.get("asset_segment", "?")
                    bstat  = f.get("benchmark_status", "NA")
                    ret1   = f.get("Return_1Y", None)
                    ret_str = f"  1yr: {ret1:.1f}%" if ret1 is not None else ""
                    print(f"      {i}. {scheme[:55]:<55}  [{seg}/{cat[:22]}]  [{bstat}]{ret_str}")
                print()

    return profile


def stage7_genai(profile: dict, risk_label: str,
                 occupation: str = "Customer") -> None:
    """
    Call explain_full_profile() for LLM/rule-based portfolio explanations.
    No personally identifiable information is passed to the LLM.
    """
    _banner("STAGE 7 — GenAI Explanation  (LLM  →  Rule-Based Fallback)")

    user_context = (
        f"A {occupation} with {risk_label.replace('_', ' ')} "
        f"risk appetite, looking to invest ₹[amount]."
    )

    print(f"\n  User context (privacy-safe): {user_context}\n")
...
def run_demo(customer_id: str, total_amount: float) -> None:
    ...
    # Retrieve occupation for GenAI context (name excluded for privacy)
    ...
    stage7_genai(profile, final_label, occ)

    # ── Summary table ─────────────────────────────────────────────────────────
    _banner("SUMMARY")
    _arrow("Customer",              customer_id)
