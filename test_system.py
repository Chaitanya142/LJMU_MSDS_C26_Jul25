...
============================================
Tests all 16 subsystems of the Smart Fund Advisor pipeline:
 ...
 14. Model Calibration — Brier Score < 0.10
 15. Fairness — Demographic Parity DIR ≥ 0.80
 16. FL-Central Accuracy Gap < 2.0 pp
...
# ─── 14. Model Calibration (Brier Score) ─────────────────────────────────────
print("\n[14/16] Model Calibration (Brier Score)")
try:
    import torch as _torch14
    import pandas as _pd14
    from src.evaluation import evaluate_calibration
    from src.central_model import RiskMLP as _RiskMLP14
    from config import RISK_FEATURES as _RF14

    _df_fl14 = _pd14.read_csv("models/df_fl_split.csv")
    _fl_model14 = _RiskMLP14(input_dim=len(_RF14))
    _fl_model14.load_state_dict(_torch14.load("models/fl_global_risk_model.pt",
                                               map_location="cpu"))
    _fl_model14.eval()

    cal_result = evaluate_calibration(_fl_model14, _df_fl14)
    bs = cal_result["brier_score"]
    assert cal_result["pass"], f"Brier Score {bs:.4f} ≥ 0.10 — model is poorly calibrated"
    assert bs < 0.10, f"Brier Score {bs:.4f} above threshold 0.10"
    assert "per_class_brier" in cal_result, "Missing per-class Brier scores"
    assert len(cal_result["per_class_brier"]) > 0, "Empty per-class Brier scores"

    print(f"      Brier Score = {bs:.4f}  (threshold < 0.10)  ✓")
    for cls, v in cal_result["per_class_brier"].items():
        print(f"        {cls:<12}: BS={v:.4f}")
...
# ─── 15. Fairness — Demographic Parity (Age Groups) ──────────────────────────
print("\n[15/16] Fairness — Demographic Parity (Age Groups)")
try:
    import torch as _torch15
    import pandas as _pd15
    from src.evaluation import evaluate_fairness
    from src.central_model import RiskMLP as _RiskMLP15
    from config import RISK_FEATURES as _RF15

    _df_fl15 = _pd15.read_csv("models/df_fl_split.csv")
    _fl_model15 = _RiskMLP15(input_dim=len(_RF15))
    _fl_model15.load_state_dict(_torch15.load("models/fl_global_risk_model.pt",
                                               map_location="cpu"))
    _fl_model15.eval()

    fair_result = evaluate_fairness(_fl_model15, _df_fl15)
    apr_val = fair_result["apr"]
    assert fair_result["pass"], (
        f"APR={apr_val:.4f} < 0.90 — model accuracy differs significantly across age groups"
    )
    assert apr_val >= 0.90, f"APR {apr_val:.4f} below accuracy-parity threshold 0.90"
    assert "per_group" in fair_result, "Missing per-group breakdown"
    grp_counts = [v["n"] for v in fair_result["per_group"].values()]
    assert all(n > 0 for n in grp_counts), "Some age groups have zero customers"

    print(f"      APR = {apr_val:.4f}  (threshold ≥ 0.90)  ✓")
    for grp, v in fair_result["per_group"].items():
...
# ─── 16. FL-Central Accuracy Gap ─────────────────────────────────────────────
print("\n[16/16] FL-Central Accuracy Gap (Privacy-Utility Trade-off)")
try:
    import torch as _torch16
    import pandas as _pd16
    from src.evaluation import evaluate_fl_central_gap
    from src.central_model import RiskMLP as _RiskMLP16, load_central_model as _load_central16
    from config import RISK_FEATURES as _RF16

    _df_fl16 = _pd16.read_csv("models/df_fl_split.csv")
    _fl_model16 = _RiskMLP16(input_dim=len(_RF16))
    _fl_model16.load_state_dict(_torch16.load("models/fl_global_risk_model.pt",
                                               map_location="cpu"))
    _fl_model16.eval()
    _central_model16 = _load_central16()

    gap_result = evaluate_fl_central_gap(_central_model16, _fl_model16, _df_fl16)
    gap_pp = gap_result["gap_pp"]
    assert gap_result["pass"], (
        f"FL-Central accuracy gap {gap_pp:.4f} pp ≥ 2.0 pp — DP noise is too destructive"
    )
    assert gap_pp < 2.0, f"Gap {gap_pp:.4f} pp above threshold 2.0 pp"

    print(f"      Gap = {gap_pp:.4f} pp  (threshold < 2.0 pp)  ✓")
    print(f"        Central={gap_result['central_acc']*100:.2f}%  "
          f"FL={gap_result['fl_acc']*100:.2f}%")
    results["fl_central_gap"] = True
except Exception as e:
    print(f"      ERROR: {e}")
    results["fl_central_gap"] = False
