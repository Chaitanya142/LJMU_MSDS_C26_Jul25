...
def compute_epsilon(
    ...
    """
    Compute (ε, δ)-DP budget for client-level DP in federated learning.

    We use Rényi DP composition (Mironov 2017) with Poisson subsampling
    amplification (Mironov et al. 2019, Theorem 9).

    Each "step" = one local SGD pass on a client device.
    Expected client participation = fraction_fit × fl_rounds.
    ...
    # Subsampling probability (per step, treating each local batch as a sub-sample)
    # For FL with batch_size=4 and dataset_size=4, q_step ≈ 1.0 per local step
    # But at the round level, device is sampled with prob = fraction_fit
    q = fraction_fit
    ...
    for alpha in alphas:
        rdp_single = _rdp_gaussian(noise_multiplier, alpha)
        # Poisson subsampling amplification bound (safe for any q)
        # RDP_sampled ≤ (1/α-1) * log(1 + binom(α,2)*q²*(exp(2*rdp) - 1))
        # Simplified conservative bound:
        amplified = min(
            q * q * rdp_single * alpha / 2.0,   # tight for small q
            rdp_single,                           # trivial (no amplification)
        )
